...
func GetMember(c *gin.Context) {
	...
	if err := c.ShouldBind(memberVo); err == nil {
		if member, err := memberSerivce.GetMember(&memberVo); err == nil {
			...
			}{UserID: strconv.FormatInt(member.UserID, 10), Nickname: member.Nickname, Username: member.UserName, UserType: member.UserType}})
		} else {
